...
The ``..`` operator calls the ``range`` function, so ``{% for i in 1..10 %}``
requires allowing the ``range`` function.
...
The following built-ins are always allowed as well:
...
Listing them in your policy is harmless.
