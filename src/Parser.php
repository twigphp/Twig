...
class Parser
{
    ...
    public function setMacro(string $name, MacroNode $node): void
    {
        if (isset($this->macros[$name])) {
            throw new SyntaxError(\sprintf('Macro "%s" is defined twice (first definition at line %d).', $name, $this->macros[$name]->getTemplateLine()), $node->getTemplateLine(), $node->getSourceContext());
        }
